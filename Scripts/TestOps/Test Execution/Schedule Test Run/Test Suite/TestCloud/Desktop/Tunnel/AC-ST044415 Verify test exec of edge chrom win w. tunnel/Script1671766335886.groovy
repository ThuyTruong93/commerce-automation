...
String testRunName = "edge-chromium-${browserVersion}-windows-private-site-" + new DateTimeUtility().getCurrentDateTime()
...
if (GlobalVariable.isNewUIScheduleEnabled) {
	...
	Page.nav(ScheduleTestRunPopUp).inputTestRunName(testRunName)
								  ...
								  .selectTestSuite(testSuitePath)
								  .clickBrowserType()
								  .selectBrowserType('Edge Chromium')
								  .clickTurnOffRepeat()
...
Page.nav(TestRunListPage).verifyTestRunName(1, testRunName)
							...
							.verifyTestRunTestEnv(1, 'TestCloud Windows Edge Chromium ' + browserVersion)
