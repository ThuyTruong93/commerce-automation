...
String testRunName = "chrome-headless-${browserVersion}-linux-private-site-" + new DateTimeUtility().getCurrentDateTime()
...
if (GlobalVariable.isNewUIScheduleEnabled) {
	...
	Page.nav(ScheduleTestRunPopUp).inputTestRunName(testRunName)
								  ...
								  .selectTestSuite(testSuitePath)
								  .clickBrowserType()
								  .selectBrowserType('Chrome (headless)')
								  .clickTurnOffRepeat()
...
Page.nav(TestRunListPage).verifyTestRunName(1, testRunName)
							...
							.verifyTestRunTestEnv(1, 'TestCloud Linux Chrome headless ' + browserVersion)
