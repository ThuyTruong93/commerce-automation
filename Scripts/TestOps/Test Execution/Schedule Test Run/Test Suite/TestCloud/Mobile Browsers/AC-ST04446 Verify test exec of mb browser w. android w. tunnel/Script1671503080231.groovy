...
String testRunName = 'mobile-browser-private-android-' +  new DateTimeUtility().getCurrentDateTime()
...
if (GlobalVariable.isNewUIScheduleEnabled) {
	...
	Page.nav(ScheduleTestRunPopUp).inputTestRunName(testRunName)
								  ...
								  .selectTestSuite(testSuitePath)
								  .clickBrowserType()
								  .selectBrowserType('Mobile Browsers')
								  .clickTurnOffRepeat()
...
Page.nav(TestRunsPage).waitUntilTestRunCompleted(1, 60 * 10)
							...
							.verifyOSIsAndroid(1)
							.verifyAndroidVersion(1, osVersionText)
