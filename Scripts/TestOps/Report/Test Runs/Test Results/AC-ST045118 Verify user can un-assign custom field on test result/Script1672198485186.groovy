...
for(customField in lstCustomFields) {
	Page.nav(TestResultPage).clickUnassignCustomField(customField.customFieldOptions.value)
}

'Verify custom field are unassigned from test results succesffuly'
for (int i = 0; i < lstCustomFields.size(); i++) {
	Page.nav(TestResultPage).verifyCustomFieldDoesNotDisplay(lstCustomFields[i].displayName, lstCustomFields[i].customFieldOptions.get(0).value)
}
